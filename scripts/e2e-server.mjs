...
import { initWorkspace } from '../dist/core/workspace.js';
...
const db = new DatabaseSync(join(root, '.codegraph', 'codegraph.db'));
db.exec(`
  ...
  INSERT INTO nodes (id,kind,name,qualified_name,file_path,language,start_line,end_line,start_column,end_column,signature,is_exported,updated_at) VALUES
    ('n-service','class','AuthService','AuthService','src/auth.ts','typescript',1,18,0,1,'class AuthService',1,2),
    ('n-login','method','login','AuthService::login','src/auth.ts','typescript',3,10,0,1,'(email: string)',0,2),
    ('n-session','function','createSession','session.createSession','src/session.ts','typescript',2,7,0,1,'(userId: string)',1,2);
